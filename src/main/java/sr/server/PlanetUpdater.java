...
public class PlanetUpdater {
    ...
    public void conqueredByTroops(Planet planet, MapPlanet mapPlanet, Player conqueringPlayer, GameWorld gameWorld, GalaxyMap galaxyMap) {
        ...
        checkVIPsOnConqueredPlanet(planet, conqueringPlayer, conqueringPlayer.getGalaxy(), galaxyMap);
    ...
    public void planetSurrenders(Planet planet, MapPlanet mapPlanet, TaskForce attackingTaskForce, Galaxy galaxy, GalaxyMap galaxyMap){
        ...
        if (playerInControl != null){
            ...
            checkVIPsOnConqueredPlanet(planet, attackingPlayer, attackingPlayer.getGalaxy(), galaxyMap);
            planet.setHasNeverSurrendered(false);

            PlanetMutator.destroyBuildingsThatCanNotBeOverTaken(planet, mapPlanet, attackingPlayer, galaxy.getGameWorld());
    	/*	if (hasSpaceStation()){
    ...
    public void razed(Planet planet, MapPlanet mapPlanet, Player aPlayer, GalaxyMap galaxyMap){
        Logger.finer("Planet.razed() called");
        Player playerInControl = planet.getPlayerInControl();
        if (playerInControl != null){
            // razeade planeter skall visas som neutrala... ingen spelare has ju kontrollen...

            PlanetMutator.setLastKnownOwner(planet.getMapPlanetUuid(), "Neutral",playerInControl.getGalaxy().turn + 1, playerInControl.getPlanetInformations());
        ...
        if (playerInControl != null){
            checkVIPsOnRazedPlanet(planet,playerInControl, playerInControl.getGalaxy(), galaxyMap);
        }else{ // check for govs on neutral planets
            checkGovernorsOnRazedPlanet(planet, aPlayer.getGalaxy(), galaxyMap);
        }
        // razed planets does not have any production or resistance
        planet.setPopulation(0);
        planet.setResistance(0);
        //attackingTF.getPlayer().getGalaxy().checkDestroyWharfs(this,attackingTF,true);
        checkDestroyBuildings(planet, mapPlanet, aPlayer, true);
        if (playerInControl != null){ // det fanns en spelare som kontrollerade planeten (dvs ej neutral)
            playerInControl.addToGeneral("");
        }
    ...
    public void checkDestroyBuildings(Planet aPlanet, MapPlanet mapPlanet, Player aPlayer, boolean autoDestroy) {
        if (autoDestroy || PlanetOrderStatusPureFunctions.isDestroyOrbitalBuildings(aPlanet.getMapPlanetUuid(), aPlayer.getPlanetOrderStatuses())) {
            ...
            for (Building building : removeBuildings) {
                ...
                aPlayer.getTurnInfo().addToLatestGeneralReport("You have destroyed a "
                        + BuildingPureFunctions.getBuildingTypeByUuid(building.getTypeUuid(), aPlayer.getGalaxy().getGameWorld()).getName() + " on " + mapPlanet.getName() + ".");
    ...
    public void checkVIPsOnRazedPlanet(Planet aPlanet, Player aPlayer, Galaxy galaxy, GalaxyMap galaxyMap) {
    ...
    public void checkGovernorsOnRazedPlanet(Planet aPlanet, Galaxy galaxy, GalaxyMap galaxyMap) {
        List<VIP> allVIPsOnPlanet = VipPureFunctions.findAllVIPsOnPlanet(aPlanet, galaxy);
        for (int i = 0; i < allVIPsOnPlanet.size(); i++) {
            VIP tempVIP = allVIPsOnPlanet.get(i);
            VIPType vipType = VipPureFunctions.getVipTypeByUuid(tempVIP.getTypeUuid(), galaxy.getGameWorld());
    ...
    public static void checkVIPsOnConqueredPlanet(Planet aPlanet, Player aPlayer, Galaxy galaxy, GalaxyMap galaxyMap) {
        List<VIP> allVIPsOnPlanet = VipPureFunctions.findAllVIPsOnPlanet(aPlanet, galaxy);
        for (int i = 0; i < allVIPsOnPlanet.size(); i++) {
            VIP tempVIP = allVIPsOnPlanet.get(i);
            if (tempVIP.getBoss() != aPlayer) {

...
public class ShipMovement implements Serializable{
  ...
  /*
  // anropas vid uppdatering av en spelares orders
  public ShipMovement(ShipMovement oldShipMove,Galaxy newGalaxy){
    destination = newGalaxy.findPlanet(oldShipMove.getDestinationName());
    ss = newGalaxy.findSpaceship(oldShipMove.getSpaceshipId());
  }*/
...
/*
  ...
  */
  ...
  public String getText(Galaxy aGalaxy){
    ...
    return "Move " + spaceShipname + " from " + aGalaxy.findSpaceship(spaceShipname, aGalaxy.getPlayer(owner)).getLocation().getName() + " to " + planetName + ".";
  }

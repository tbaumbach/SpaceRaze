...
//    private boolean ranked = false;
    ...
    /**
     * Only used to load old games. Always use auto update.
     * steps is not used when loading, set to -1
     * maxPlayers is not used when loading, set to -1
     * @param nameOfGame
     * @param port
     */
    public SR_Server(String nameOfGame,int port,ServerHandler sh){
    	this("load",null,nameOfGame,"not used when loading",port,0,true,-1,false,-1,null,null,false,false,null,sh,null,false,false, 0,0,0,0, null);
    }
    
    /**
     * Use this to start a new game that updates when a set time has passed.
     * Game will also update when all players are done with their turn.
     * @param command
     * @param nameOfGame
     * @param port
     * @param time
     */ 
    public SR_Server(String command,GameWorld aGameWorld,String nameOfGame,String nameOfMap,int id,int steps,boolean autoBalance,long time,int maxPlayers,String startedByPlayer,String gamePassword, boolean groupFaction, boolean randomGame, List<String> selectableFactionNames,ServerHandler sh, DiplomacyGameType diplomacyGameType,boolean bSinglePlayer,boolean bRanked, int singleVictory, int factionVictory, int endTurn, int numberOfStartPlanet, StatisticGameType statisticGameType){
    	this(command,aGameWorld,nameOfGame,nameOfMap,id,time,true,steps,autoBalance,maxPlayers,startedByPlayer,gamePassword,groupFaction,randomGame,selectableFactionNames,sh,diplomacyGameType,bSinglePlayer,bRanked, singleVictory, factionVictory, endTurn, numberOfStartPlanet, statisticGameType);
    }

    
    /**
     * Use this to start a new game that updates automatically when all players is 
     * finished with a turn
     * Not used any more!?
     * @param command
     * @param nameOfGame
     * @param port
     */
    /*
    public SR_Server(String command,String nameOfGame,String nameOfMap,int id,int steps,boolean autoBalance){
    ...
    */
    ...
    private SR_Server(String command,GameWorld aGameWorld, String nameOfGame,String nameOfMap,int id,long aTime,boolean autoUpdate,int steps,boolean autoBalance,int maxPlayers, String startedByPlayer, String gamePassword, boolean groupFaction, boolean randomGame, List<String> selectableFactionNames,ServerHandler sh, DiplomacyGameType diplomacyGameType,boolean bSinglePlayer, boolean bRanked, int singleVictory, int factionVictory, int endTurn, int numberOfStartPlanet, StatisticGameType statisticGameType){
...
//    	this.startedByPlayer = startedByPlayer;
    	...
    	parseCommand(autoBalance,maxPlayers,startedByPlayer,aGameWorld,gamePassword,groupFaction,randomGame,selectableFactionNames,diplomacyGameType,bSinglePlayer,bRanked, singleVictory, factionVictory, endTurn, numberOfStartPlanet, statisticGameType);
    }
    ...
    private void parseCommand(boolean autoBalance,int maxPlayers,String startedByPlayer,GameWorld aGameWorld,String gamePassword,boolean groupFaction, boolean randomGame, List<String> selectableFactionNames, DiplomacyGameType diplomacyGameType,boolean singleplayer, boolean ranked,  int singleVictory, int factionVictory, int endTurn, int numberOfStartPlanet, StatisticGameType statisticGameType){
        if (command.equalsIgnoreCase("create")){
            createGalaxy(autoBalance,maxPlayers,startedByPlayer,aGameWorld,gamePassword,groupFaction,randomGame,selectableFactionNames,diplomacyGameType, singleplayer, ranked, singleVictory, factionVictory, endTurn, numberOfStartPlanet, statisticGameType);
        ...
        if (command.equalsIgnoreCase("update")){ // never used anymore???
            try {
				updateGalaxy(true);
			} catch (Exception e) {
...
//    }
    ...
    private void createGalaxy(boolean autoBalance,int maxPlayers,String startedByPlayer,GameWorld aGameWorld,String gamePassword,boolean groupFaction, boolean randomGame, List<String> selectableFactionNames, DiplomacyGameType diplomacyGameType,boolean singleplayer, boolean ranked, int singleVictory, int factionVictory, int endTurn, int numberOfStartPlanet, StatisticGameType statisticGameType){
        // when planetlist is in a file, send the filename as the second parameter
        // filename can be a parameter to the .bat file
        g = gc.createGalaxy(nameOfGame,nameOfMap,steps,aGameWorld, singleVictory, factionVictory, endTurn, numberOfStartPlanet, statisticGameType);
...
//            	Thread.dumpStack();
            ...
            if (command.equalsIgnoreCase("newplayer")){
              if (g.getTurn() > 0){
                ...
                if (!factionIsOpenAndSelectable(factionName)){
                	p = new Player("All slots in the " + factionName + " faction have just been taken. Choose another faction.");
                }else{
                    p = g.getNewPlayer(name,password,govenorName,factionName);
                }
              }
            }else{
        ...
        if ((st.countTokens() == 1) && st.nextToken().equals("checkStatus")){
            p = new Player("Returning status.",g);
        }else
        if ((st.countTokens() == 2) && st.nextToken().equals("update")){
        ...
        if (msg.equalsIgnoreCase("Player data inserted successfully.")){
          ...
          if (g.turn > 0){
    		...
    		if (this.singlePlayer == true)
    		{
    			...
    			p2 = g.getNewPlayer("SpaceRaze","EzarEcaps","SpaceRaze",factionName);
    			g.replacePlayer(p2);
//    			String msg2 = g.replacePlayer(p2); vet inte vad msg2 skulle användas till?
    			gs.saveGalaxy(nameOfGame,"saves",g);
    			gs.saveGalaxy(nameOfGame + "_" + g.getTurn(),"saves/previous",g);
    			updateGalaxy(true);
    		}
...
/*
	...
	public static void main (String[] args){
    	...
    	System.out.println(sdf.format(gameCal.getTime()));
    	Calendar nowCal = Calendar.getInstance();
    	System.out.println(sdf.format(nowCal.getTime()));
    	if (nowCal.after(gameCal)){
    		okToDelete = true;
        	System.out.println("now after game");
    	}else{
